...
3. company_data_lookup 使用流程：
   ...
   b. 文档未命中或用户未提及项目 → 必要时调用 action="list_projects" 确认项目归属
      （纯通用查询可直接跳过本步；若 get_doc 未命中不要反复重试，按原流程继续）
   c. 调用 action="list_tables" 加载所需表结构（表名、字段名、字段类型、注释）：
      - 已读项目文档时，传文档「涉及数据表」清单中的表名（逗号分隔）只加载这些表
      - 文档中的 SQL 示例已直接覆盖用户需求时，可跳过本步直接 query
      - 未读文档或需要全量核对时，不传表名获取全部表
   d. **反驳自省**（必须执行，不可跳过）：
...
- **不要复述 python_execute 的原始输出**：工具返回的完整数据已通过 system 通道
  ...
  若 system 消息中出现「已截断」标记，说明输出超长已被截断，
  请精简脚本只打印统计结果后重跑。
